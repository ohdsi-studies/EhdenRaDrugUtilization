...
with flMono as(
	select database, year, step_1, totalcohortcount, SUM(personcount) tot_personcount
	from public.network_pathways_results
	where step_1 NOT LIKE '%+%'
	group by database, year, step_1, totalcohortcount
), comboOfInterest AS (
	select database, year, '[EHDEN RA] methotrexate use + [EHDEN RA] hydroxychloroquine use' step_1, totalcohortcount, SUM(personcount) tot_personcount
	from public.network_pathways_results
	where step_1 IN (
		'[EHDEN RA] methotrexate use + [EHDEN RA] hydroxychloroquine use',
		'[EHDEN RA] hydroxychloroquine use + [EHDEN RA] methotrexate use'
	)
	group by database, year, totalcohortcount
), uncensored AS (
	select database, year, step_1, totalcohortcount, tot_personcount
	from flMono
	...
	select a.database, year, step_1, totalcohortcount, SUM(tot_personcount) tot_personcount
	from (
		SELECT database, year, step_1, totalcohortcount, tot_personcount
		FROM flMono
		WHERE tot_personcount < 10
	...
	select database, year, 'COMBO' step_1, totalcohortcount, SUM(personcount) tot_personcount
	from public.network_pathways_results
	where step_1 LIKE '%+%'
	  and step_1 NOT IN (
		'[EHDEN RA] methotrexate use + [EHDEN RA] hydroxychloroquine use',
	...
	select database, year, totalcohortcount, SUM(personcount) total_patients_with_paths
	from public.network_pathways_results
	group by database, year HAVING SUM(personcount) < 10
)
SELECT r.*
